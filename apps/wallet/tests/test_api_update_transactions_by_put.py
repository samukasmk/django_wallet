...
def test_update_transactions_by_put(api_client, sample_transactions_models):
    ...
    for transaction_to_get in transactions_to_get:
        ...
        if amount_transaction > 0:
            transaction_to_get['amount'] = str(amount_transaction + 10000000.0)
        ...
        assert FinancialTransaction.objects.get(
            reference=transaction_to_get["category"]).amount == 'updated_category_by_put'
